...
int growproc(int n)
{
  ...
  if (curproc->thread_count == -1)
  {
    ...
    else
    {
      for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
      {
        if (p != curproc && p->parent == curproc->parent && p->thread_count == -1)
        {
          p->sz = curproc->sz;
          child_count--;
        }
  ...
  {
    child_count = curproc->thread_count - 1;
    if (child_count <= 0)
    {
      release(&ptable.lock);
      release(&thread);
    ...
    else
    {
      for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
      {
        if (p->parent == curproc && p->thread_count == -1)
        {
          p->sz = curproc->sz;
          child_count--;
        }
...
int has_children(struct proc *process)
{
  struct proc *p;
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
